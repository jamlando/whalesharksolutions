...
export default function AnimatedServicesSection() {
  ...
  const getCurrentFinPosition = (serviceId: string) => {
    ...
    // Calculate position based on the actual grid layout
    // The grid uses max-w-5xl (1024px) with 4 columns and gap-8 (32px)
    ...
    // Center the whale shark icon (96px wide) under each column
    const iconOffset = 48 // Half of 96px icon width
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8 mb-2 max-w-5xl mx-auto relative">
          {services.map((service) => (
            <WaterRipple key={service.id} onClick={() => {
              setPreviousService(selectedService)
              setSelectedService(service)
            }}>
              <div className="flex flex-col items-center justify-center h-[240px] pt-8">
                <div className="flex flex-col items-center justify-center w-full">
                  {/* Service Icon */}
                  <div className={`w-20 h-20 md:w-24 md:h-24 z-10 transition-all duration-300 flex items-center justify-center ${
                  ...
                  {/* Service Title */}
                  <div className="text-center mt-2 px-2 min-h-[70px] flex items-center justify-center w-full">
          ...
          {/* Single Animated Whale Shark that moves between services */}
          <motion.div
            className="absolute top-[200px] left-0 z-20"
            animate={{
          ...
          >
            <img 
              ...
              alt="Whale Shark"
              className="w-24 h-24 transition-all duration-300 scale-x-[-1] brightness-0 invert sepia saturate-10000 hue-rotate-[340deg] drop-shadow-[0_0_20px_rgba(249,159,129,0.9)] drop-shadow-[0_0_40px_rgba(249,159,129,0.6)] drop-shadow-[0_0_8px_rgba(0,0,0,1)] drop-shadow-[0_0_12px_rgba(0,0,0,0.8)]"
            />
          </motion.div>
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className={`bg-gradient-to-r ${selectedService.gradient} p-8 rounded-2xl shadow-xl relative`}>
              {/* Ocean Wave Effect */}
                ...
                <svg className="w-full h-full" viewBox="0 0 100 12" preserveAspectRatio="none">
                  <defs>
                    <linearGradient id="waveGradient" x1="0%" y1="0%" x2="100%" y2="0%">
                      <stop offset="0%" stopColor={selectedService.id === 'web-design' ? '#f99f81' : selectedService.id === 'tool-implementation' ? '#f4b5a0' : selectedService.id === 'automation' ? '#e8a5a5' : '#f4b5a0'} />
                      <stop offset="50%" stopColor={selectedService.id === 'web-design' ? '#f99f81' : selectedService.id === 'tool-implementation' ? '#f4b5a0' : selectedService.id === 'automation' ? '#e8a5a5' : '#f4b5a0'} />
                      <stop offset="100%" stopColor={selectedService.id === 'web-design' ? '#f99f81' : selectedService.id === 'tool-implementation' ? '#f4b5a0' : selectedService.id === 'automation' ? '#e8a5a0' : '#f4b5a0'} />
                    </linearGradient>
                  </defs>
                  <path 
